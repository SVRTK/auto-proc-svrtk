...
if [[ $# -ne 2 ]] ; then
    echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-brain-bounti-segmentation-fetal-gpu.sh"
...
python3 ${segm_path}/src/run_monai_comb_red_atunet_unet_segmentation-2022-lr-gpu.py ${main_dir}/ ${monai_check_path_bounti_attunet}/ ${monai_check_path_bounti_unet}/ masked-stack-info.json ${main_dir}/monai-segmentation-results-bounti ${res} ${monai_lab_num}
...
if [[ ${number_of_stacks} -eq 0 ]];then
    echo
    echo "-----------------------------------------------------------------------------"
    echo "ERROR: GPU BOUNTI SEGMENTATION DID NOT WORK !!!!" - RUNNING CPU
    echo "-----------------------------------------------------------------------------"
    echo
