...
echo "-----------------------------------------------------------------------------"
echo
echo "SVRTK for fetal MRI (KCL): auto uterus segmentation for bSSFP / BTFE / TRUFI fetal MRI"
echo "Source code: https://github.com/SVRTK/auto-proc-svrtk"
echo
echo "Please cite: Neves Silva, S., Uus, A., Waheed, H., Bansal, S., St Clair, K.,"
echo "Norman, W., Aviles Verdera, J., Cromb, D., Woodgate, T., van Poppel, M., "
echo "Steinweg, J. K., Matthew, J., Pushparajah, K., Lloyd, D., Kyriakopoulou, V., "
echo "Siassakos, D., David, A., Hajnal, J. v, Story, L., … Hutter, J. (2026). "
echo "Scanner-based real-time automated volumetry reporting of the fetus, amniotic "
echo "fluid, placenta, and umbilical cord for fetal MRI at 0.55T. Magnetic Resonance "
echo "in Medicine, 95(2), 975–986. https://doi.org/10.1002/mrm.70097"
echo
echo "-----------------------------------------------------------------------------"
echo "-----------------------------------------------------------------------------"
echo
...
if [[ $# -ne 2 ]] ; then
    echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-uterus-internal-segmentation-btfe.sh"
    echo "            [full path to the folder with bSSFP / TRUFI stacks in .nii.gz]"
...
echo
echo "-----------------------------------------------------------------------------"
echo "REMOVING NAN & NEGATIVE/EXTREME VALUES & REMOVING DYNAMICS & N4 ..."
echo "-----------------------------------------------------------------------------"
echo
...
do
    ...
    /home/auto-proc-svrtk/bin/N4BiasFieldCorrection -i ${s} -o tmp.nii.gz  > t.txt
    mv tmp.nii.gz ${s}
    ${mirtk_path}/mirtk nan ${s} 1000000
...
echo "-----------------------------------------------------------------------------"
echo
echo "3D UTERUS SEGMENTATION OF BSSPF STACKS ..."
echo
...
monai_check_path_uterus_unet=${segm_path}/trained_models/monai-checkpoints-unet-uterus-bsspf-5-lab


mkdir monai-segmentation-results-uterus
python3 ${segm_path}/src/run_monai_unet_segmentation-2022.py ${main_dir}/ ${monai_check_path_uterus_unet}/ stack-info.json ${main_dir}/monai-segmentation-results-uterus ${res} ${monai_lab_num}


number_of_stacks=$(find monai-segmentation-results-uterus/ -name "*.nii*" | wc -l)
if [[ ${number_of_stacks} -eq 0 ]];then
    echo
    echo "-----------------------------------------------------------------------------"
...
out_mask_names=$(ls monai-segmentation-results-uterus/cnn-*.nii*)
...
do
    ...
    ${mirtk_path}/mirtk transform-image ${all_masks[$i]} uterus-masks/mask-${jj}.nii.gz  -target ${all_stacks[$i]} -labels
    

    ${mirtk_path}/mirtk transform-and-rename ${all_stacks[$i]} uterus-masks/mask-${jj}.nii.gz "-mask-uterus-5" ${main_dir}/final-masks
