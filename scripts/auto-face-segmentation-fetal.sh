...
#fetal
monai_check_path_face_attunet=${segm_path}/trained_models/monai-checkpoints-attunet-face-1-lab
...
if [[ $# -ne 2 ]] ; then
    echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-face-segmentation-fetal.sh"
...
echo "-----------------------------------------------------------------------------"
echo
echo "3D FACE SEGMENTATION OF 3D SVR T2W BRAIN RECONS..."
echo
...
do
    ...
    ${mirtk_path}/mirtk extract-label ${all_masks[$i]} face-masks/mask-${jj}.nii.gz 1 1
    ${mirtk_path}/mirtk extract-connected-components face-masks/mask-${jj}.nii.gz face-masks/mask-${jj}.nii.gz
...
do
    ...
    ${mirtk_path}/mirtk transform-image face-masks/mask-${jj}.nii.gz face-masks/mask-${jj}.nii.gz  -target ${all_stacks[$i]} -labels
    
    ${mirtk_path}/mirtk smooth-image face-masks/mask-${jj}.nii.gz face-masks/mask-${jj}.nii.gz -float
    
    ${mirtk_path}/mirtk threshold-image face-masks/mask-${jj}.nii.gz face-masks/mask-${jj}.nii.gz 0.5  > t.txt

    ${mirtk_path}/mirtk transform-and-rename ${all_stacks[$i]} face-masks/mask-${jj}.nii.gz "-mask-face-1" ${main_dir}/final-masks
