...
echo "-----------------------------------------------------------------------------"
echo
echo "SVRTK for fetal MRI (KCL): auto head SVR reconstruction for 1.5-3T TSE T2w fetal MRI"
...
if [ $# -ne 2 ] ; then
    ...
    if [ $# -ne 6 ] ; then
        echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-head-reconstruction.sh"
...
do
    ...
    ${mirtk_path}/mirtk dilate-image recon-masks-head/mask-${jj}.nii.gz recon-masks-head/mask-${jj}.nii.gz -iterations 2
...
#        ${mirtk_path}/mirtk nan average_mask_cnn.nii.gz 0
    ...
    ${mirtk_path}/mirtk reconstruct tmp-output.nii.gz  ${number_of_stacks} ../proc-stacks-head/*.nii* -mask ../average_mask_cnn.nii.gz -template ../transf-selected_template.nii.gz -default_thickness ${default_thickness} -svr_only -structural -iterations 1 -bg_dilation ${bg_dl} -resolution 1.6 -with_background
    

    if [ $motion_correction_mode -eq 1 ]; then
    
        ${mirtk_path}/mirtk reconstruct ../SVR-output-head.nii.gz ${number_of_stacks} ../proc-stacks-head/*.nii* -mask ../average_mask_cnn.nii.gz -template tmp-output.nii.gz -default_thickness ${default_thickness} -iterations 3  -resolution ${recon_resolution} -bg_dilation ${bg_dl} -structural -svr_only -with_background
        
    else
    
        ${mirtk_path}/mirtk reconstruct ../SVR-output-head.nii.gz ${number_of_stacks} ../proc-stacks-head/*.nii* -mask ../average_mask_cnn.nii.gz -template tmp-output.nii.gz -default_thickness ${default_thickness} -structural -iterations 3  -resolution ${recon_resolution} -bg_dilation ${bg_dl} -svr_only -with_background
