...
atl=${template_path}/reo-brain-template-042026
...
if [ $# -ne 2 ] ; then
    ...
    if [ $# -ne 6 ] ; then
        echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-brain-055t-reconstruction-2026.sh"
...
echo "-----------------------------------------------------------------------------"
echo 
echo "RUNNING GLOBAL BRAIN LOCALISATION AND REORIENTATION ..."
echo
...
do
    ...
    res=128 ; lab_num=1 ;
    current_monai_check_path=${model_path}/monai-2026/at_unet_bet_brain_055t_stacks_2026_128_best_metric_model.pth
    python3 ${segm_path}/src/run_monai_atunet_segmentation_1case-2024.py ${res} ${lab_num} ${current_monai_check_path} res-global-stacks/stack-${jj}.nii.gz out-global-masks/mask-${jj}.nii.gz

    ${mirtk_path}/mirtk extract-connected-components out-global-masks/mask-${jj}.nii.gz out-global-masks/mask-${jj}.nii.gz

    ${mirtk_path}/mirtk dilate-image out-global-masks/mask-${jj}.nii.gz out-global-masks/dl-mask-${jj}.nii.gz -iterations 1
    ...
    res=128 ; lab_num=5 ;
    current_monai_check_path=${model_path}/monai-2026/at_unet_reo_brain_5lab_global_full_rois_055t_v3_2026_128_best_metric_model.pth
    python3 ${segm_path}/src/run_monai_atunet_segmentation_1case-2026-flip.py ${res} ${lab_num} ${current_monai_check_path} res-crop-stacks/crop-stack-${jj}.nii.gz out-reo-masks/reo-mask-${jj}.nii.gz
   ...
   #  ${mirtk_path}/mirtk extract-label out-reo-masks/reo-mask-${jj}.nii.gz l${q}.nii.gz 2 4
    ...
    ${mirtk_path}/mirtk init-dof i.dof 
   ${mirtk_path}/mirtk register-landmarks ${atl}/v3-l1.nii.gz l1.nii.gz i.dof proc-files/d-${jj}.dof 4 4 ${atl}/v3-l1.nii.gz ${atl}/v3-l2.nii.gz ${atl}/v3-l3.nii.gz ${atl}/v3-l4.nii.gz l1.nii.gz l2.nii.gz l3.nii.gz l4.nii.gz  > t.txt 
...
${mirtk_path}/mirtk stacks-and-masks-selection-2026 selected-template.nii.gz selected-mask.nii.gz recon-files 20 700 ${number_of_stacks} proc-files/org-stack-*.nii.gz proc-files/org-mask-*.nii.gz proc-files/d-*.dof
...
if [ -f ${test_file} ];then
    ...
    # ${mirtk_path}/mirtk crop-image selected-template.nii.gz selected-mask.nii.gz cropped-selected-template.nii.gz
    # ${mirtk_path}/mirtk edit-image ${atl}/dl-ref-1mm.nii.gz tmp-ref.nii.gz -copy-origin cropped-selected-template.nii.gz
...
${mirtk_path}/mirtk dilate-image dl-selected-mask.nii.gz dl-selected-mask.nii.gz -iterations 2
${mirtk_path}/mirtk mask-image SVR-output.nii.gz dl-selected-mask.nii.gz masked-SVR-output.nii.gz
${mirtk_path}/mirtk crop-image masked-SVR-output.nii.gz dl-selected-mask.nii.gz masked-SVR-output.nii.gz
${mirtk_path}/mirtk pad-3d masked-SVR-output.nii.gz pad-res-masked-SVR-output.nii.gz 128 1 


res=128 ; lab_num=5 ; 

current_monai_check_path=${segm_path}/trained_models/monai-checkpoints-unet-svr-brain-reo-5-lab/best_metric_model.pth
python3 ${segm_path}/src/run_monai_unet_segmentation_1case-2024.py  ${res} ${lab_num} ${current_monai_check_path} pad-res-masked-SVR-output.nii.gz reo-mask-for-SVR-output.nii.gz 


test_file=reo-mask-for-SVR-output.nii.gz
if [ ! -f ${test_file} ];then
    chmod 1777 -R ${output_main_folder}/
    echo
    echo "-----------------------------------------------------------------------------"
    echo "ERROR: SVR REORIENTATION DID NOT WORK !!!!"
    echo "-----------------------------------------------------------------------------"
    echo
    exit
fi
...
for ((q=1;q<6;q++));
do
    ${mirtk_path}/mirtk extract-label reo-mask-for-SVR-output.nii.gz out-svr-reo-masks/mask-${q}.nii.gz ${q} ${q}
...
${mirtk_path}/mirtk register-landmarks ${template_path}/brain-ref-atlas-2022/mask-${z1}.nii.gz ${all_org_stacks[$j]} init.dof dof-to-atl.dof ${n_roi} ${n_roi}  ${template_path}/brain-ref-atlas-2022/mask-${z1}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z2}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z3}.nii.gz ${template_path}/brain-ref-atlas-2022/mask-${z4}.nii.gz out-svr-reo-masks/mask-${z1}.nii.gz out-svr-reo-masks/mask-${z2}.nii.gz out-svr-reo-masks/mask-${z3}.nii.gz out-svr-reo-masks/mask-${z4}.nii.gz > tmp.log
...
${mirtk_path}/mirtk transform-image SVR-output.nii.gz reo-SVR-output.nii.gz -target ref.nii.gz -dofin dof-to-atl.dof -interp BSpline
${mirtk_path}/mirtk threshold-image reo-SVR-output.nii.gz tmp-m.nii.gz 0.01 > tmp.txt 
${mirtk_path}/mirtk crop-image reo-SVR-output.nii.gz tmp-m.nii.gz reo-SVR-output.nii.gz
${mirtk_path}/mirtk nan reo-SVR-output.nii.gz 100000
${mirtk_path}/mirtk convert-image reo-SVR-output.nii.gz reo-SVR-output.nii.gz -rescale 0 5000 -short

test_file=reo-SVR-output.nii.gz
if [ ! -f ${test_file} ];then
    chmod 1777 -R ${output_main_folder}/
    echo
    echo "-----------------------------------------------------------------------------"
...
cp -r reo-SVR-output.nii.gz ${output_main_folder}/
chmod 1777 -R ${output_main_folder}/

test_file=${output_main_folder}/reo-SVR-output.nii.gz
if [ ! -f ${test_file} ];then
