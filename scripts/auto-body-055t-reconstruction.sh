...
echo "-----------------------------------------------------------------------------"
echo
echo "SVRTK for fetal MRI (KCL): auto body DSVR reconstruction for 0.55T TSE T2w fetal MRI"
...
if [ $# -ne 2 ] ; then
    ...
    if [ $# -ne 6 ] ; then
        echo "Usage: bash /home/auto-proc-svrtk/scripts/auto-body-055t-reconstruction.sh"
...
# ${mirtk_path}/mirtk average-images ave-space-cropped.nii.gz masked-cropped-stacks-${roi}/stack-*.nii.gz 
...
#     ${mirtk_path}/mirtk transform-image ${all_org_stacks[$i]}  masked-cropped-stacks-${roi}/stack-${jj}.nii.gz  -target ave-space-cropped.nii.gz
#     ${mirtk_path}/mirtk mask-image masked-cropped-stacks-${roi}/stack-${jj}.nii.gz dl-m.nii.gz masked-cropped-stacks-${roi}/stack-${jj}.nii.gz
#     ${mirtk_path}/mirtk crop-image masked-cropped-stacks-${roi}/stack-${jj}.nii.gz dl-m.nii.gz masked-cropped-stacks-${roi}/stack-${jj}.nii.gz 
...
#   -delta 110 -lambda 0.018 -lastIter 0.008 ;
    ...
    ${mirtk_path}/mirtk transform-image DSVR-output-${recon_roi}.nii.gz reo-DSVR-output-${recon_roi}.nii.gz -target ref.nii.gz -dofin dof-to-atl-${recon_roi}.dof -interp BSpline
    ${mirtk_path}/mirtk threshold-image reo-DSVR-output-${recon_roi}.nii.gz tmp-m.nii.gz 0.01 > tmp.txt 
    ${mirtk_path}/mirtk crop-image reo-DSVR-output-${recon_roi}.nii.gz tmp-m.nii.gz reo-DSVR-output-${recon_roi}.nii.gz
    ${mirtk_path}/mirtk nan reo-DSVR-output-${recon_roi}.nii.gz 100000
    ...
    if [ ${number_of_final_recons} -ne 0 ];then
        ...
        cp -r reo-DSVR-output-${recon_roi}.nii.gz ${output_main_folder}/
